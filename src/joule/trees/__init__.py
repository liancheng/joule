...
from joule.maybe import head_or_none, just, maybe
...
class Paren(Expr):
    ...
    @staticmethod
    def from_cst(node: ts.Node) -> Expr:
        assert node.type == "parenthesized"
...
class FieldKey(Tree):
    ...
    def from_cst(node: ts.Node) -> FieldKey:
        assert node.type in ("static_key", "computed_key")
        return (
...
class ObjComp(Expr):
    ...
    def from_cst(node: ts.Node) -> ObjComp:
        ...
        key = just(node.child_by_field_name("key"))
        params = node.child_by_field_name("parameters")
        value_or_body = just(node.child_by_field_name("value_or_body"))
        ...
        return ObjComp(
            ...
            for_spec=ForSpec.from_cst(just(node.child_by_field_name("for_spec"))),
            binds=binds,
